...
const generateboard = (gameBoardSize, player1, player2, allShips) => {
    ...
    const attackShip = (e, player1, player2) => {
        ...
        if (tile == "playerOne" && player1.checkTurn()) {
        ...
        } else if (tile == "playerTwo" && player2.checkTurn()) {
            e.target.classList.add("hit");
            const hitNum = e.target.getAttribute("data-id") + e.target.getAttribute("data-key");
            totalHits.push(hitNum);
            checkHits(player1, player2, allShips, totalHits);
    ...
    const tileClick = function(e){
        if (player1.checkTurn() && !e.target.classList.contains("hit")) {
        ...
        } else if (player2.checkTurn() && !e.target.classList.contains("hit")) {
            if (attackShip(e, player1, player2)) {
    ...
    gameContainer1.addEventListener('click', function(e){
        tileClick(e)
    })

    gameContainer2.addEventListener('click', function(e){
        tileClick(e)
    })
